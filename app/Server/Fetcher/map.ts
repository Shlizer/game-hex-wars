...
export default class MapFetch {
    static init() {
        ipcMain.on('map-list-request', (event : IpcMainEvent) => event.reply('map-list-data', this.getList()))
    }
    ...
    static getList(): TypeInfo[] {
        ...
        if (fs.existsSync(this.getMainDir)) {
            fs.readdirSync(this.getMainDir).map((mapDir: string) => {
                const paths = this.getDirs(mapDir)

                if (fs.existsSync(paths.map) && fs.existsSync(paths.info) && fs.existsSync(paths.layout)) {
                    const info = Object.assign({}, {id: mapDir, ...fs.readJSONSync(paths.info)})
                    maps.push(info)
                } else {
